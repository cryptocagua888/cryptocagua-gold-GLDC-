...
const App: React.FC = () => {
  ...
  const orderDetails = useMemo(() => {
    ...
    return { amount, subtotal, fee, total, gramPrice: gold.gramPrice };
  ...
  const handleNotify = () => {
    ...
    const body = `DETALLES DE LA OPERACIÓN CRYPTOCAGUA GOLD
------------------------------------------------
Tipo de Orden: ${orderType === 'BUY' ? 'ADQUISICIÓN (Compra)' : 'LIQUIDACIÓN (Venta)'}
Cantidad: ${orderDetails.amount} Gramos de GLDC
Precio Spot: $${orderDetails.gramPrice.toFixed(4)} USDT/g
------------------------------------------------
Subtotal: $${orderDetails.subtotal.toFixed(2)} USDT
Comisión de Transferencia (0.75%): $${orderDetails.fee.toFixed(2)} USDT
MONTO FINAL ESTIMADO: $${orderDetails.total.toFixed(2)} USDT
------------------------------------------------
Billetera del Usuario: ${wallet.address}
Billetera de Tesorería Destino: ${TREASURY_WALLET}
------------------------------------------------
Por favor, procesen mi solicitud. Adjunto comprobante si aplica.`;
    ...
    <div className="min-h-screen pb-20">
      {wrongNetwork && wallet.isConnected && (
        <div className="bg-red-600 text-white px-6 py-4 flex flex-wrap items-center justify-center gap-4 text-[10px] font-black sticky top-0 z-[100] shadow-2xl">
          ...
          <span className="tracking-widest uppercase">Red Incorrecta. Cambia a Binance Smart Chain.</span>
          <button 
          ...
          <div className="gold-gradient p-12 rounded-[4.5rem] text-black shadow-gold-lg relative overflow-hidden">
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-4">
                ...
                {wallet.isConnected && <button onClick={() => {}} className="p-2 bg-black/5 hover:bg-black/15 rounded-2xl transition-all"><PlusCircle size={22} /></button>}
              </div>
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 backdrop-blur-3xl bg-black/90">
          <div className="glass w-full max-w-2xl p-12 sm:p-16 rounded-[4rem] sm:rounded-[5rem] border border-yellow-500/20 relative shadow-gold-lg animate-in fade-in zoom-in duration-300">
            <button onClick={() => setShowConfirm(false)} className="absolute top-8 right-8 sm:top-12 sm:right-12 text-white/20 hover:text-white transition-all"><X size={32}/></button>
            
            <div className="text-center mb-10">
              <h3 className="text-3xl sm:text-5xl font-black uppercase tracking-tighter mb-2">Desglose de Orden</h3>
              <p className="text-[10px] font-bold text-yellow-500/50 uppercase tracking-[0.4em]">Transacción de Oro Físico</p>
            </div>

            <div className="space-y-8">
              <div className="bg-black/60 rounded-[3rem] border border-white/5 overflow-hidden">
                <div className="p-8 border-b border-white/5">
                  <div className="flex justify-between items-center text-[11px] font-black uppercase tracking-widest text-white/30 mb-4">
                    <span>Detalle</span>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-white/60">Monto ({orderDetails.amount}g GLDC)</span>
                      <span className="text-lg font-bold">${orderDetails.subtotal.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm font-medium text-yellow-500/60">Comisión Token (0.75%)</span>
                      <span className="text-lg font-bold text-yellow-500">${orderDetails.fee.toLocaleString(undefined, {minimumFractionDigits: 2})}</span>
                    </div>
                  </div>
                </div>
                
                <div className="p-10 bg-yellow-500/5 text-center">
                  <p className="text-[11px] font-black text-yellow-500 uppercase mb-2 tracking-[0.3em]">Total Final Estimado</p>
                  <h4 className="text-6xl sm:text-7xl font-black tracking-tighter">${orderDetails.total.toLocaleString(undefined, {minimumFractionDigits: 2})}</h4>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] ml-4">Dirección de Billetera de Tesorería</p>
                <div className="bg-black/80 p-6 rounded-3xl border border-white/10 flex items-center justify-between gap-4 group hover:border-yellow-500/30 transition-all cursor-pointer" onClick={() => copyToClipboard(TREASURY_WALLET)}>
                  <code className="text-[11px] sm:text-xs font-mono text-white/60 break-all select-all">{TREASURY_WALLET}</code>
                  <div className="shrink-0 p-3 bg-white/5 rounded-xl group-hover:bg-yellow-500 transition-all group-hover:text-black">
              ...
              <div className="pt-6">
                <button 
                  ...
                  className="w-full py-8 sm:py-10 gold-gradient text-black rounded-[2.5rem] sm:rounded-[3rem] font-black uppercase text-[12px] sm:text-[13px] tracking-[0.3em] flex items-center justify-center gap-5 shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
                  Enviar Notificación <Send size={22}/>
                </button>
                <p className="text-[9px] text-center text-white/20 font-black uppercase tracking-[0.2em] mt-6 leading-relaxed">
                  Al confirmar, se notificará a tesorería para procesar la entrega física del activo.
                </p>
              </div>
            </div>
          </div>
